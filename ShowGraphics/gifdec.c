...
gd_open_gif(const char *fname)
{
    ...
    /* Read GCT */
    gif->gct.size = gct_sz;
    read(fd, gif->gct.colors, 3 * (size_t)gif->gct.size);
    gif->palette = &gif->gct;
    gif->bgindex = bgidx;
...
new_table(int key_size)
{
    ...
    Table *table = (Table *) malloc(sizeof(*table) + sizeof(Entry) * (unsigned long)init_bulk);
    if (table) {
...
add_entry(Table **tablep, uint16_t length, uint16_t prefix, uint8_t suffix)
{
    ...
    if (table->nentries == table->bulk) {
        table->bulk *= 2;
        table = (Table *) realloc(table, sizeof(*table) + sizeof(Entry) * (unsigned long)table->bulk);
        if (!table) return -1;
        table->entries = (Entry *) &table[1];
        *tablep = table;
    }
...
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    clear = (uint16_t)(1 << (uint8_t)key_size);
    stop = clear + 1;
...
read_image(gd_GIF *gif)
{
    ...
    if (fisrz & 0x80) {
        ...
        read(gif->fd, gif->lct.colors, 3 *(size_t)gif->lct.size);
        gif->palette = &gif->lct;
    } else
        gif->palette = &gif->gct;
    /* Image Data. */
